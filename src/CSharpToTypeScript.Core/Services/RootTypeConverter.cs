...
internal class RootTypeConverter(TypeConversionHandler typeConverter)
{
    private readonly TypeConversionHandler _typeConverter = typeConverter;
    ...
    public RootTypeNode Convert(TypeDeclarationSyntax type)
        => new RootTypeNode(
            ...
            fields: type.ChildNodes()
                ...
                {
                    PropertyDeclarationSyntax property when IsSerializable(property, type) => [ConvertProperty(property)],
    ...
    private IEnumerable<TypeNode> ConvertBaseTypes(IEnumerable<BaseTypeSyntax> baseTypes, TypeDeclarationSyntax containingType)
    {
        ...
        if (containingType is not InterfaceDeclarationSyntax)
        {
    ...
    private bool IsNotStatic(MemberDeclarationSyntax member)
        => member.Modifiers.All(m => !m.IsKind(SyntaxKind.StaticKeyword));
    ...
    private bool IsPublic(MemberDeclarationSyntax property, TypeDeclarationSyntax? containingType = null)
        ...
        || property.Modifiers.Any(m => m.IsKind(SyntaxKind.PublicKeyword));

    private bool IsGettable(PropertyDeclarationSyntax property)
        => property.AccessorList is null
        || (HasGetter(property, out var getter) && getter != null && !HasRestrictedAccess(getter));
